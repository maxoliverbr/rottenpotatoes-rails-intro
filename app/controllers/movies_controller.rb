class MoviesController < ApplicationController
  ...
  def index
    ...
    if params[:ratings].nil? and params[:sort].nil?
      if not session[:sort].nil? and not session[:ratings_to_show].nil?
        redirect_to movies_path(sort: session[:sort], ratings: session[:ratings_to_show])
      else
        redirect_to movies_path(sort: "title", ratings: {"G"=>"1", "PG"=>"1", "PG-13"=>"1", "NC-17"=>"1", "R"=>"1"})
      end
    end
